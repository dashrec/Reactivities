...
const Profiles = {
  ...
  uploadPhoto: (file: Blob) => { let formData = new FormData();
  ...
  listFollowings: (username: string, predicate: string) => requests.get<Profile[]>(`/follow/${username}?predicate=${predicate}`), // we are gonna be returning list of Profile
